...
# Student Deployment Documentation

**Branch:** `feature/s10arnaud-docker-compose-project`
**Student:** s10Arnaud
...
The following must be installed on the machine before running any commands:

- Docker Engine (version 20.10 or later)
- Docker Compose plugin (version 2.x, included with Docker Desktop)
- Git
...
The files added on this branch are:

```
docker-compose.yml        # Defines and connects all 11 services
.env                      # Holds the shared database password (not committed)
src/ui/Dockerfile         # Java 17 Dockerfile for the UI service
src/cart/Dockerfile       # Java 17 Dockerfile for the carts service
src/orders/Dockerfile     # Java 17 Dockerfile for the orders service
src/catalog/Dockerfile    # Go Dockerfile for the catalog service
src/checkout/Dockerfile   # Node.js Dockerfile for the checkout service
src/assets/Dockerfile     # Nginx Dockerfile (pre-existing, unmodified)
```
...
Before starting the application, create a `.env` file in the project root (same directory as `docker-compose.yml`) with the following content:

```
MYSQL_PASSWORD=store-local-pass
```

This password is shared by all MariaDB-backed services. The `.env` file is excluded from Git by `.gitignore` and must be created manually on each machine.

---

## How to Build the Images

Build all six application images from source:

```bash
docker compose build
```

To build a single service image:

```bash
docker compose build ui
docker compose build catalog
docker compose build carts
docker compose build orders
docker compose build checkout
docker compose build assets
```

The first build will take several minutes because Maven downloads dependencies and compiles the Java services inside the container.
...
Check that all 11 services are running:

```bash
docker compose ps
```

All services should show a status of `running` or `healthy`. The database services (`catalog-db`, `orders-db`) and `rabbitmq` have health checks. Dependent services will not start until those health checks pass.
...
```
# To be updated after server deployment
```
...
The `-v` flag removes the named volumes (`catalog-db-data`, `orders-db-data`). Omit it if you want to preserve database data between restarts.
...
### 1. Checkout Dockerfile was the wrong runtime

The `src/checkout/Dockerfile` initially contained a copy of the Java Dockerfile. The checkout service is built with Node.js and NestJS, not Java.

**Resolution:** Replaced the file with the correct Node.js Dockerfile from `images/nodejs/Dockerfile`.
...
![docker compose up output](Screenshots/compose_up.png)

---

### docker ps — all containers running

![docker ps showing all 11 containers](Screenshots/docker_ps.png)

---

### Store Homepage

![Retail store homepage at http://localhost:8888](Screenshots/homepage.png)

---

### Catalog Page

![Product catalog page at http://localhost:8888/catalog](Screenshots/catalog.png)
